...
export const fetchPosts = () => {
    return async (dispatch: any) => {
        ...
        try {
            // async
            const response = await fetch("https://reactproject-a0334-default-rtdb.europe-west1.firebasedatabase.app/posts"); //URL to posts
            if (!response.ok) {
...
export const addComment = (newComment: any, postId:string) => {
    return async (dispatch:any, getState:any) => {
        try {
            ...
            const response = await fetch(`https://reactproject-a0334-default-rtdb.europe-west1.firebasedatabase.app/posts/${postId}.json`, {
                method: "PATCH",
                headers: {
...
export const likeComment = (userId:string, commentId:string, postId:string) => {
    return async (dispatch:any, getState:any) => {
        try {
            // get all posts from the state
            const posts = getState().posts.posts;
            ...
            const index = comments.findIndex((comment:Comment) => comments.id == commentId);
...
export const dislikeComment = (userId:string, commentId:string, postId:string) => {
    return async (dispatch:any, getState:any) => {
        try {
            // get all posts from the state
            const posts = getState().posts.posts;
...
export const deletePost = (id:string) => {
    return async (dispatch: any, state: any) => {
        ...
        try {
            ...
            dispatch({ type: DELETE_POST, posts: loadedPosts });
        } catch (error) {
            throw error;
        }
    };
